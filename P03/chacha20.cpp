 ...
 */
std::vector<uint32_t> initializeState(const std::vector<uint8_t>& key, uint32_t counter, const std::vector<uint8_t>& nonce) {
 ...
 */
std::vector<uint8_t> chacha20Encrypt(const std::vector<uint8_t>& plaintext, const std::vector<uint8_t>& key, uint32_t counter, const std::vector<uint8_t>& nonce) {
  // Inicializamos el estado.
  auto state = initializeState(key, counter, nonce);
...
int main() {
  ...
  // Convertimos el mensaje a un vector de bytes.
  std::vector<uint8_t> plaintext(message.begin(), message.end());

  // Realizamos el cifrado y mostramos el resultado en hexadecimal.
  auto ciphertext = chacha20Encrypt(plaintext, key, counter, nonce);
  std::cout << "Cifrado: ";
  ...
  auto decrypted = chacha20Encrypt(ciphertext, key, counter, nonce);
